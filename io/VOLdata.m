classdef VOLdata < handle
    %VOLDATA class for Synchrotron CT Data input
    ...
    methods (Static=true)
        ...
        function [data, header, filename] = load(filename)
            %Load VolFloat32 file
            
            % get filename if no file is given as input
            if nargin == 0
                [filename, pathname] = uigetfile({'*.vol;*.VOL','Synchrotron CT Data Files (*.vol,*.VOL)';'*.*', 'All Files (*.*)'},'Load VolFloat32 data');
                if filename==0
                    % user pressed cancel
                    return;
                end
            ...
            if interactive
                ...
                validateattributes(size_x,  {'numeric'}, {'nonnegative','integer','nonempty','finite'}, mfilename,'size_x',3);
                validateattributes(size_y,  {'numeric'}, {'nonnegative','integer','nonempty','finite'}, mfilename,'size_y',3);
                validateattributes(size_z,  {'numeric'}, {'nonnegative','integer','nonempty','finite'}, mfilename,'size_z',3);
                
            end
            
            % read data
            data = ISQdata.readdata(filename, x_min, y_min, z_min, size_x, size_y, size_z);
        ...
        function data = readdata(filename, x_min, y_min, z_min, size_x, size_y, size_z)
            ...
            data = zeros(size_x,size_y,size_z,'int16');
        ...
        %             fprintf('jj=%i\n',jj);
                    ...
                    % Read line
                    data(:, jj, kk) = fread(fid, size_x, 'int16');
                    % Seek everything after the last column we want to read
                    fseek(fid, (headerinfo.x_dim-(x_min+size_x-1))*2, 0);
                end
                % Seek everything after the last line we want to read
                fseek(fid, headerinfo.x_dim*(headerinfo.y_dim-(y_min+size_y-1))*2, 0);
            end
            fclose(fid);
            fprintf(' done!\n');
